...
class AGTServer:
    ...
    async def handle_new_client_connection(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            ...
            #validate game type
            if not player_game_type or player_game_type != self.game_config["name"]:
                print(f"Invalid game type response from {address}")
                await self.send_message(writer, {
                    "message": "error",
    ...
    async def run_tournament(self):
        ...
        self.debug_print(f"==========================================")
        self.debug_print(f"run_tournament called with {len(self.players)} players")
        self.debug_print(f"==========================================")
        
        try:
            game_title = self.game_config['name']
            print(f"TOURNAMENT {game_title} started with {len(self.players)} players", flush=True)
            
            #set up all the agents for the tournament by creating a NetworkAdapter for each player.
            #this allows the game to actually communicate with the connected clients.
            from server.network_adapter import NetworkAdapter
            agents = [NetworkAdapter(player.name, player.writer, player.reader) for player in self.players.values()]
            ...
            await self._send_tournament_results(list(self.players.values()), results_df, arena.agent_stats)
            
            print(f"TOURNAMENT {game_title} ended.", flush=True)
            ...
            self.debug_print(f"==========================================")
            self.debug_print(f"run_tournament method completed")
            self.debug_print(f"==========================================")
